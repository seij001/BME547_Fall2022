def create_patient_entry(patient_name, patient_id,
    ...
    #The empty bracket is an extra list that is a placeholder for now; we will store test result here
    return new_patient
...
def main():
    ...
    for i, patient in enumerate(db): #enumerate also tells what index I am iterating on
        print(i)
        print("Name = {}, Room: {}".format(patient[0], room))

    for patient, room in zip(db, room_list): #use zip when you want to iterate over two loops of same size
        print("Name = {}, Room: {}".format(patient[0], room))
